...
import { readFigFile, readPenFile } from '@open-pencil/core'
...
export async function openFileInNewTab(
  ...
  const isPen = file.name.toLowerCase().endsWith('.pen')
  const imported = isPen
    ? await readPenFile(file)
    : await readFigFile(file)
  const sourceFormat = isPen ? 'pen' : 'fig'
